...
export async function GET() {
  try {
    const data = await withTransaction(async (client) => {
      ...
      const mediaAttachmentsData = await client.query('SELECT * FROM media_attachments');
      const reviewsData = await client.query('SELECT * FROM reviews');
      const projectActivitiesData = await client.query('SELECT * FROM project_activities');
      
      // If you have users table
      let usersData;
      try {
        usersData = await client.query('SELECT id, name, email, created_at FROM users');
      } catch (error) {
